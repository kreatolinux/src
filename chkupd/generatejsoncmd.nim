...
proc generateJson*(repo: string, limit = 256, splitIfLimit = true,
        output = "out.json", ignorePackages = @[Update(packages: "")],
        instance = 1) =
  ...
  var count: int
  var runFiles: seq[string] = @[]
  for f in walkFiles(repoFullPath&"/*/run3"): runFiles.add(f)
  for f in walkFiles(repoFullPath&"/*/run"): runFiles.add(f)

  var processedPkgs: seq[string] = @[]

  for runFile in runFiles:

    let pkg = lastPathPart(runFile.parentDir)

    if pkg in processedPkgs:
      continue
    ...
    if count >= limit and limit > 0:
      if splitIfLimit:
        ...
        generateJson(repo, limit, splitIfLimit, output = outputSplit[
                0]&"-"&($(instance + 1))&"."&outputSplit[1],
                ignorePackages = ignorePackages&updateList, instance = (
                instance + 1))
    ...
    var deps = parseRun3(runFile.parentDir).getDepends().join(
            " ") # Not full dependency list ofc, but good enough for now
