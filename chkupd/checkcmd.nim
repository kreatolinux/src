...
proc matchesWildcard*(text: string, pattern: string): bool =
  ...
  while textIdx < text.len:
    if patternIdx < pattern.len and (pattern[patternIdx] == '?' or pattern[
            patternIdx] == text[textIdx]):
      textIdx += 1
      patternIdx += 1
...
proc check*(package: string, repo: string, backend: string, autoUpdate = true,
  ...
  if '*' in package or '?' in package:
    ...
    for runFile in walkFiles(repoFullPath&"/*/run"):
      let pkg = lastPathPart(runFile.parentDir)
      if matchesWildcard(pkg, package) and pkg notin matchedPackages:
        matchedPackages.add(pkg)
  ...
  case actualBackend:
    ...
    of "githubReleases":
      if githubReleasesRepo == "":
        ...
        quit(1)
      githubReleasesCheck(package, repo, githubReleasesRepo, autoUpdate,
              skipIfDownloadFails, trimString, verbose)
    else:
