...
proc enable(service: seq[string], now=false) =
    ...
    connectSock()
    if now:
        sendSock("""{ "client": { "name": "Jumpstart CLI", "version": """"&clCfg.version&"""" }, "service": { "name": """"&service[0]&""".service", "action": "enable", "now": "true" }}""")
    else:
        sendSock("""{ "client": { "name": "Jumpstart CLI", "version": """"&clCfg.version&"""" }, "service": { "name": """"&service[0]&""".service", "action": "enable", "now": "false" }}""")
...
proc disable(service: seq[string], now=false) =
    ...
    connectSock()
    if now:
        sendSock("""{ "client": { "name": "Jumpstart CLI", "version": """"&clCfg.version&"""" }, "service": { "name": """"&service[0]&""".service", "action": "disable", "now": "true" }}""")
    else:
        sendSock("""{ "client": { "name": "Jumpstart CLI", "version": """"&clCfg.version&"""" }, "service": { "name": """"&service[0]&""".service", "action": "disable", "now": "false" }}""")
    echo("jumpstart: disabled service "&service[0])

proc start(service: seq[string]) =
    ## Start service.
    connectSock()
    sendSock("""{ "client": { "name": "Jumpstart CLI", "version": """"&clCfg.version&"""" }, "service": { "name": """"&service[0]&""".service", "action": "start", "now": "false" }}""")
    echo("jumpstart: started service "&service[0])

proc stop(service: seq[string]) =
    ## Stop service.
    connectSock()
    sendSock("""{ "client": { "name": "Jumpstart CLI", "version": """"&clCfg.version&"""" }, "service": { "name": """"&service[0]&""".service", "action": "stop", "now": "false" }}""")
