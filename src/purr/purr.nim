...
proc purr(tests = "all", tmpdir = "/tmp/purr") =
    ...
    if dephandler(toSeq(["sway"])).join(" ") != "pcre expat openssl zlib libxcrypt python libxml2 ninja meson wayland samurai wayland-protocols libpciaccess libdrm xkeyboard-config libxkbcommon libevdev mtdev gmake libudev libinput seatd pixman libelf python-mako perl m4 autoconf automake libtool libuv libzstd linux-headers libarchive nghttp2 curl cmake llvm flex mesa wlroots libpng freetype-harfbuzz pkgconf cairo git glib fribidi gperf fontconfig pango json-c":
