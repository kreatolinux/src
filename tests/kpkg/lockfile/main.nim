...
suite "lockfile":
  ...
  test "logging preserves callback and ownership until cleanup":
    ...
    # Logging cannot drop exclusion while rollback/cleanup still runs.
    check fileExists(lockfilePath)
    check previousFired
    removeLockfile()
  ...
  test "removeLockfile removes owned lockfile":
    createLockfile()
    check fileExists(lockfilePath)
