...
proc printResult(repo: string, package: string): string =
    ...
    if dirExists("/var/cache/kpkg/installed/"&package):
      r = "i   "
      pkgrunf = parse_runfile("/var/cache/kpkg/installed/"&package)
    else:
      r = "ni  "
      pkgrunf = parse_runfile("/etc/kpkg/repos/"&repo&"/"&package)
...
proc search*(keyword: seq[string]) =
    ...
    for r in getConfigValue("Repositories", "repoDirs").split(" "):
      setCurrentDir(r)
      for i in walkDirs("*"):
        if (fuzzyMatchSmart(i, keyword[0]) >= 0.6 or fuzzyMatchSmart(parse_runfile(r&"/"&i).desc, keyword.join(" ")) >= 0.8) and i != keyword[0]:
