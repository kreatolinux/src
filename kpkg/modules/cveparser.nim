...
func newVulnerability*(cve = "", description = "", package = "",
    versionEndExcluding = ""): vulnerability =
  vulnerability(cve: cve, description: description, package: package,
      versionEndExcluding: versionEndExcluding)

proc updateVulns*(dbConn: DbConn, file: string,
    removeFileAfterwards = false): int =
  ...
  for i in json["cve_items"]:
    ...
    for i in 0..cpeMatch.len:
      try:
        versionEndExcluding = versionEndExcluding&cpeMatch[i][
            "versionEndExcluding"].getStr()
      except Exception:
        versionEndExcluding = versionEndExcluding&"false"

    var vulnFinal = newVulnerability(cve = cve, description = description,
        package = package, versionEndExcluding = versionEndExcluding.join("::"))
