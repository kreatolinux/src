...
proc createPackage*(actualPackage: string, pkg: runFile, kTarget: string): string =
    ...
    for file in toSeq(walkDirRec(kpkgBuildRoot, {pcFile, pcLinkToFile, pcDir, pcLinkToDir})):
        ...
        if dirExists(file) or symlinkExists(file):
            ...
            dict.setSectionKey("", "\""&relativePath(file, kpkgBuildRoot)&"\"", getSum(file, "b2"))
