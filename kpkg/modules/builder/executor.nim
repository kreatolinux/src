...
proc executeBuildSteps*(ctx: ExecutionContext, state: BuildState,
  ...
  if state.exists.prepare:
    if executeRun3Stage(ctx, state.pkg.run3Data.parsed, "prepare") != 0:
      fatal("prepare failed")

  # Determine and execute "build"
  let buildFunc = getBuildFuncName(state.exists, actualPackage)
  if buildFunc != "":
    if executeRun3Stage(ctx, state.pkg.run3Data.parsed, buildFunc) != 0:
      fatal("build failed")

  # Execute "check" (tests) if requested
  if tests and state.exists.check:
    if executeRun3Stage(ctx, state.pkg.run3Data.parsed, "check") != 0:
      fatal("check failed")
  ...
  if executeRun3Stage(ctx, state.pkg.run3Data.parsed, pkgFunc) != 0:
